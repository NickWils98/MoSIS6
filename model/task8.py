...
if __name__ == '__main__':
    ...
    system = model.PortSystem(det_bool=True, generation_max=4)
    sim = Simulator(system)
    sim.setTerminationTime(55)
    sim.setClassicDEVS()
    sim.simulate()
    ...
    f.write(f"\n\nAverage travel time for a vessel: {stat1}")
    f.write(f"\n\nAverage waiting time for a vessel in the anchorpoint: {stat2}")
    f.write(f"\n\nAverage number of vessels in the port: {stat3}")
    f.write(f"\n\nTotal vessels in the port at every hour in the simulation: \n{stat4}")
    f.write(f"\n\nAverage idle time for Lock A: {stat5A}")
    f.write(f"\n\nAverage idle time for Lock B: {stat5B}")
    f.write(f"\n\nAverage idle time for Lock C: {stat5C}")
    ...
    plt.savefig(f"{filename}stat4.png")
    plt.close()
    ...
    plt.savefig(f"{filename}stat7A.png")
    plt.close()
    ...
    plt.savefig(f"{filename}stat7B.png")
    plt.close()
    ...
    plt.savefig(f"{filename}stat7C.png")
    plt.close()